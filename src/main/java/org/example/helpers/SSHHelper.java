...
public class SSHHelper {
    ...
    /**
     * Execute the given command on a remote host over SSH
     *
    ...
    public static List<String> sshAndRunCommand(Session session, String command) {
    ...
    public static void scp(Session session, String localFilePath, String remoteDirectory) {
        boolean preserveOriginalFileTimestamps = true;
        ChannelExec channel = null;

        try {
            // exec 'scp -t rfile' remotely
            String command = "scp " + (preserveOriginalFileTimestamps ? "-p" : "") + " -t " + remoteDirectory;
            channel = (ChannelExec) session.openChannel("exec");
            channel.setCommand(command);
            ...
            if (preserveOriginalFileTimestamps) {
